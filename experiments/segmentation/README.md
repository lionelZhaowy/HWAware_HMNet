...
分支 `seg_rgbdvs_640x440_v2.0` 基于 `main/c573491`。四个stage在原融合模块入口加入逐元素乘加：
...
保留前置IRB＋Concat更新及注意力归一化和参数。支持 **BF16单卡 / FP32多卡**，与另一新实验使用相同公共训练代码。全部启动、续训、评估命令和精度说明见 **[训练精度与多卡说明](TRAINING_PRECISION.md)**。

- BF16输出：`logs/segmentation/efficientvit_b1_cross_v20_bf16/`。
- FP32双卡输出：`logs/segmentation/efficientvit_b1_cross_v20_fp32_ddp2/`。

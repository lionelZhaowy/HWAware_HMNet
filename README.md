...
三任务复用骨干结构、分别训练，不是共享参数的多任务学习。先在分割实验确定事件帧表示、骨干、融合及后续时序自回归方案，再逐项迁移至检测/深度。当前代码使用 RVT Histogram、官方 B1，分割保留四尺度投影相加基线，并新增方案 C：标准归一化 LiteMLA 双分支阶段交互（独立配置 `efficientvit_b1_cross.py`）。HWAware、S-FIFO、自回归尚未接入。

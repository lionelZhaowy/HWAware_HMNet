...
三任务复用骨干结构、独立实例化训练。当前工程为 **v1.2_T.1：50ms双极性二值输入**，分支 `seg_rgbdvs_640x440_v1.2_T.1`，直接派生自同步v1.2_T/76f1a8c。DVS输入为2通道0/1占用图，RGB同步编码；保持Add、DVS LiteMLA M=2、TBPTT=1及150epoch/BF16训练。只有必要DVS首层输入通道从20改为2，非首层初始化与父版一致。与异步下划线版本v1.2_T_1不同。

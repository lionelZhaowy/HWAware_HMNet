...
三任务复用骨干结构、独立实例化训练。当前工程为 **HWAware_HMNet_Seg_RGBDVS_640x440_v3**，分支 `seg_rgbdvs_640x440_v3`，默认融合 `adaptive_add`，BF16、150 epoch。结构说明、验证范围与训练/恢复命令见[融合实验说明](experiments/segmentation/TRAINING_PRECISION.md)。保留原HMNet、检测/深度入口；当前模型尚未加入HWAware、S-FIFO或自回归。
